...
data.to_csv(f'{folder_name}/Data_exp_1_{7}_dim.csv', index=False)
...
# We save the results as a dataframe.
data = pd.DataFrame(results)
data.to_csv(f'{folder_name}/Data_exp_2_dim.csv', index=False)
...
for d, exp_num in zip([30, 40, 50], [4, 7, 10]):
    d_head = d
    min_width = 2*d*(1-1)
    max_width = 2*d*(21-1)
    step = 2*d*5
    ...
    # We save the results as a dataframe.
    data = pd.DataFrame(results)
    data.to_csv(f'{folder_name}/Data_exp_5_{exp_num}_dim.csv', index=False)
...
for d, exp_num in zip([7, 13], [4, 10]):
    ...
    # We save the results as a dataframe.
    data = pd.DataFrame(results)
    data.to_csv(f'{folder_name}/Data_exp_1_{exp_num}_dim.csv', index=False)